...
## Gate P0-3 frontend — 2026-06-28 — 403 « Token invalide » = session morte (DEF-1)

**Session :** correctif DEF-1 (RAPPORT-TESTS-FONCTIONNELS.md) — seul le 401 purgeait la session ; un 403 « Token invalide » (rotation `JWT_SECRET`) tombait en mode hors-ligne → l'utilisateur restait bloqué avec des données locales périmées. Tâche non-🔒 auto-déployable → commit direct sur `main`.
**Fix :** classification centralisée dans `lib/api.ts` — `isDeadAuthError` (401, ou 403 dont le corps cite token/jwt/signature, « csrf »/« xsrf » exclus en priorité) → purge + retour login ; `isNetworkFailure` (status 0/502/503/504) → mode hors-ligne + retry. `lib/app-context.tsx` (`loadData`) : branche session morte appelle `logout()` (efface AUSSI repas/glycémie/poids/activités/produits scannés, pas seulement le token) ; offline réservé aux vraies pannes réseau. Un 403 non-auth (CSRF/métier) ne purge pas.
**Fichiers :** `lib/api.ts` (NEW helpers `isDeadAuthError`/`isNetworkFailure`), `lib/app-context.tsx` (`loadData` catch), `lib/__tests__/p0-3-dead-auth.test.ts` (NEW — 23 TU : classification pure + chemin HTTP réel msw sur /journal/query).
**Verdict build :** GO — `npm run build` OK · `tsc --noEmit` 0 erreur
**Verdict tests :** GO — 256/256 (23 TU-P0-3 : 403 Token invalide→déconnexion, 503→offline non-régression, 403 CSRF→pas de purge, invalid signature→déconnexion, corps vide→offline prudent)
**Verdict revue-code :** GO — après 1 itération. MAJEUR (regex `/token|jwt/` ratait « invalid signature » brut de jsonwebtoken → bug réapparaîtrait sous un autre libellé) corrigé : ajout de `signature`, exclusion `xsrf`, TU dédiés. Non-régression 401 confirmée (y compris 401 fabriqués par `guardArray`). Ordre du catch correct (dead-auth avant network-failure, `return` immédiat).
**Verdict réglementaire :** GO — après 1 itération. MAJEUR (purge partielle laissant `scannedProducts`/4 tableaux santé/PII en mémoire → régression vs ancien chemin offline + risque de contamination inter-utilisateurs même session sans reload) corrigé : la branche session morte appelle `logout()` (minimisation RGPD/REG-03). REG-05 amélioré : plus d'affichage indéfini de données périmées. REG-04 N.A. : aucun texte utilisateur ajouté (logique + commentaires seuls). Aucune donnée santé en `localStorage` (seuls `token`/`language`).
**Résiduels MINEURS (non bloquants) :** classification d'un 403 non-auth tombe en offline sans déconnexion (défaut prudent assumé) ; `isDiabetic` non réinitialisé par `logout()` (comportement pré-existant de la déconnexion volontaire, hors périmètre).
**Dette suivie (autre repo, hors boucle frontend) :** DEF-4 (logger `err.name`/`err.message` dans le `catch` de `auth.js` backend) et DEF-2 (double-secret transitoire sur rotation `JWT_SECRET`) restent côté `nutridz`.
